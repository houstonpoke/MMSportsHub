...
FALLBACK_ODDS = [
    {"name": "Scottie Scheffler", "price": +400},
    {"name": "Rory McIlroy", "price": +900},
    {"name": "Jon Rahm", "price": +800},
    {"name": "Brooks Koepka", "price": +2000},
    {"name": "Collin Morikawa", "price": +3000}
]
...
odds_data = get_real_odds(sport="golf", market="h2h")
...
if not displayed:
    st.warning("⚠️ Live odds unavailable. Showing model predictions with fallback odds.")

    for outcome in FALLBACK_ODDS:
        player = outcome["name"]
        odds = outcome["price"]
        implied = calculate_implied_probability(odds)
        model_prob = model_win_prob(player)
